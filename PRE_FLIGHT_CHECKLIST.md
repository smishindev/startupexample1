...
**Last Updated**: March 7, 2026 - Instructor Public Profile Page complete — public API service without auth interceptors, privacy check `!== 'public'` pattern, `disableBottomPad={!isAuthenticated}` for mixed-auth pages, `RouterLink` on Typography for instructor names, URL `.trim()` in SQL params; Coupon patterns also added 🏫
...
- [x] **Mobile Optimization Phase 19 — Typography, Stat Cards, Icons & Final Sweep** - PRODUCTION READY (Feb 28, 2026) 📱
  - 23 files fixed across 2 rounds + final confirming sweep. 0 TypeScript errors. 3-pass scan confirmed 0 remaining issues.
  - **h3 stat numbers must be responsive**: `variant="h3"` inside stat cards/summary sections needs `sx={{ fontSize: { xs: '1.75rem', sm: '3rem' } }}`. Without this, the MUI default `3rem` overflows on 375px screens.
  - **h4 stat numbers must be responsive**: `variant="h4"` inside stat cards needs `sx={{ fontSize: { xs: '1.5rem', sm: '2.125rem' } }}`. MUI default `2.125rem` is too large on mobile next to icons.
  - **Stat card grids — use `xs={6}` not `xs={12}` for number cards**: 4-card overview rows should use `xs={6} sm={6} md={3}` (2-per-row on mobile, 2-per-row on tablet, 4-per-row on desktop). Single `xs={12}` stat cards look like full-width banners on phones.
  - **Large icons/Avatar sizing must be responsive**: Icons with `fontSize >= 48` or Avatars with `width/height >= 48` should use `fontSize: { xs: smaller, sm: original }`. E.g., `fontSize: { xs: 48, sm: 64 }` or Avatar `width: { xs: 40, sm: 56 }`.
  - **Table column hiding (alternative to Table→Card)**: For wide tables where full Card conversion is impractical, add `display: { xs: 'none', md: 'table-cell' }` to secondary-data columns (Type, Time, secondary metrics). Keep essential columns (Name, Status, Progress, Actions) always visible.
...
- [x] **Instructor Public Profile Page** - PRODUCTION READY (March 7, 2026) 🏫
  - Public profile page at `/instructor/:instructorId` (no auth), `InstructorProfileService.ts`, `GET /api/instructors/:id/profile`, `InstructorProfilePage.tsx` (404 lines). 2 audit bugs found and fixed. 0 TypeScript errors.
  - **Public API service — no auth interceptor**: `instructorProfileApi.ts` uses an axios instance without the `authenticateToken` interceptor. Any API service for a page accessible without login must NOT add auth interceptors — attaching auth to a public endpoint causes it to fail for logged-out users.
  - **Privacy check must use `!== 'public'`**: when building a public-facing endpoint that should only expose profiles with `ProfileVisibility = 'public'`, always check `!== 'public'` (not `=== 'private'`). Values like `'students'` or `'private'` both need to be blocked. `=== 'private'` only blocks one value and leaks others.
  - **`disableBottomPad={!isAuthenticated}` on mixed-auth pages**: public pages that also work while authenticated (e.g., public profile) should use `disableBottomPad={!isAuthenticated}` on `PageContainer`. This correctly omits bottom padding when MobileBottomNav is absent (guest) and adds it when it is present (authenticated user).
  - **`RouterLink` on Typography / instructor names**: to make a text element a clickable link without losing its `variant` styling, use MUI's polymorphic `component` prop: `<Typography component={RouterLink} to="/instructor/:id" sx={{ textDecoration: 'none', color: 'inherit' }}>`. Do NOT nest a raw `<a>` inside Typography.
  - **`onClick={(e) => e.stopPropagation()}`**: when adding a `RouterLink` inside a parent clickable element (e.g., CourseCard), always stop event propagation to prevent the card's click handler from also firing.
  - **URL params must be trimmed in SQL object**: when accepting URL input (WebsiteUrl, LinkedInUrl, TwitterUrl), add `?.trim()` in the SQL parameters object, not just in the `new URL()` validation call. The validation trim is discarded; only the trimmed value in the params is stored.
  - **5 instructor-profile DB columns are all nullable**: Bio, Headline, WebsiteUrl, LinkedInUrl, TwitterUrl were added as nullable — existing rows need no backfill. Idempotent migration: `IF COL_LENGTH('dbo.Users','Bio') IS NULL` guard before each `ALTER TABLE ADD COLUMN`.
  - **`pt: 2` not `pt: 4` on InstructorProfilePage**: the hero section is a full-width `Box` with no `PageContainer` top margin, so content starts at the very top of the page. Use `pt: 2` (not `pt: 4`) to avoid excess whitespace between the AppBar and the hero.
  - Files (new): `server/src/services/InstructorProfileService.ts`, `server/src/routes/instructorProfile.ts`, `client/src/services/instructorProfileApi.ts`, `client/src/pages/Instructor/InstructorProfilePage.tsx`, `database/add_instructor_profile_fields.sql`
  - Files (modified): `server/src/routes/profile.ts`, `server/src/index.ts`, `server/src/types/database.ts`, `client/src/App.tsx`, `client/src/services/profileApi.ts`, `client/src/pages/Profile/ProfilePage.tsx`, `client/src/pages/Course/CourseDetailPage.tsx`, `client/src/components/Course/CourseCard.tsx`, `database/schema.sql`
